...
cache_dir = "cache"
_special_cases = json.load(open(__location__ + os.sep + "special_cases.json", "r"))
...
def get_func_loc(func_name, version="v4.14"):
    ...
    if response.status_code == 200:
        ...
        for h2_tag in h2_tags:
            if "Defined in" in h2_tag.text and "as a function" in h2_tag.text:
                ...
                for li_tag in li_tags:
                    href_url: str = li_tag.find("a").get("href")
                    file_location = href_url.replace(f"/linux/{version}/", "", 1)
                    if file_location.startswith("/"):
                        print(
                            "Warning: href prefix may not start with /linux/{{version}}: href = ",
                            href_url,
                            file=sys.stderr,
                        )
        ...
        print(
            "Error: Unable to fetch the content from the function: ",
            func_name,
            file=sys.stderr,
        )
